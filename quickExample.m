...
rng(123); % Fix the RNG seed, for reproducibility
...
Cp = ParkerAif(t,bolusArrivalTime);

% Simulate tissue curves
Ct = ToftsKety(Cp,[ktrans kep vp],t,1);
...
termAbove = CrrTail - CrrTail(1) + estKepRR * cumtrapz(tTail,CrrTail);
termBelow = cumtrapz(tTail,CpTail);
estKtransRR = termBelow\termAbove;
% (Might look odd to have Below\Above instead of Above\Below, but that's how it is)
...
disp([estTofts(1), estTofts(1)/estTofts(2), estTofts(3)])
...
disp('Simulated ground truth for reference tissue [ktransRR, veRR]')
disp([ktransRR, veRR])

disp('RRIFT estimates for reference tissue [ktransRR, veRR]')
